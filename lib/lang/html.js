...
var rStyle = /<!--([\s\S]*?)(?:-->|$)|(?:(<link[^>]*(?:\/)?>)|(<style[^>]*>([\s\S]*?)<\/style>))(<!--ignore-->)?/ig;
...
function obtainFramework(content, resource, opts) {
  ...
  return content.replace(rScript, function(all, comment, script, body, ignored) {
    ...
    if (!body.trim() && rSrcHref.test(script)) {
      ...
      // 则认为此资源为前端loader
      if (rLoader.test(script) || file && ~['require.js', 'esl.js', 'mod.js'].indexOf(file.basename)) {

        // 根据 js 自动设置 resourceType。
        if (opts.resourceType === 'auto') {
          opts.resourceType = file && file.basename === 'mod.js' ? 'mod' : 'amd';
        }
...
function init(file, resource, opts) {
  ...
  content = process.obtainFramework(content, resource, opts);
...
function beforePack(file, resource, opts) {
  ...
  if (!opts.useInlineMap) {
    ...
    if (~idx) {
      var resoucemap = resource[/mod|commonJs/i.test(opts.resourceType) ? 'buildResourceMap' : 'buildAMDPath']();
      if (!resoucemap) {
